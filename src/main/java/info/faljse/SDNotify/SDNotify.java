...
public class SDNotify {
    ...
    private SDNotify() {
        ...
        if (socketName == null || socketName.length() == 0) {
            System.out.println("WARNING: [info.faljse.SDNotify.SDNotify] Environment variable " +
                NOTIFY_SOCKET + " not set. Cannot notify SystemD.");
            return;
        }
        try {
            ...
            System.err.println("WARNING: [info.faljse.SDNotify.SDNotify] SDNotify init failed. Cannot notify SystemD.");
            e.printStackTrace();
        }
        available = true;
    }
